...
bool atapiplayer_update( atapiplayer_t * player )
{
	if( atapi_testUnitReady() )
	{
		if( !( player->status & ATAPIPLAYER_STATUS_CDINSERTED ) )
		{
			...
			{
				printf_P( PSTR(" %c Track %02d  qADRControl 0x%02X  Start %02d:%02d:%02d\n"),
			...
			{
				printf_P( PSTR("No Audio Tracks found\n") );
				return false;
			}
		...
		if( player->status & ATAPIPLAYER_STATUS_PLAYING && !(player->status & ATAPIPLAYER_STATUS_PAUSED) )
		{
			...
			{
				printf_P( PSTR("Track %02d  Index %02d  Absolute %02d:%02d:%02d  Relative %02d:%02d:%02d  State 0x%02X  qADRControl 0x%02X\r"),
